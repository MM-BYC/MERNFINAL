...
const STEPS = [
  ...
  },
  {
    ...
    title: 'Check & Delete',
    text: 'Click the box to mark an item with ✓, then click the Delete button on that card to remove it',
...
export default function Tutorial({ onClose }) {
  ...
  const recalc = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setPos({
        spot: { top: r.top - 8, left: r.left - 8, width: r.width + 16, height: r.height + 16 },
        bubble: { top, left },
        arrow,
      });
    }, 300);
  ...
  return (
    <div className="tut-root">
      ...
      >
        <div className="tut-count">Step {step + 1} of {STEPS.length}</div>
